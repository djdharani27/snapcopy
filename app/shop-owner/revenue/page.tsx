...
export default async function ShopOwnerRevenuePage() {
  ...
  return (
    <DashboardShell
      ...
      description="Track paid orders and the amount routed to your linked account."
      hideIntro
    ...
    >
      ...
      {dailyRows.length === 0 ? (
        <div className="panel p-8 text-center text-sm text-slate-600">
          No paid orders yet. Revenue appears after verified payments.
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Date</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Paid orders</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Revenue</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200 bg-white">
                {dailyRows.map((row) => (
                  <tr key={row.date}>
                    <td className="px-5 py-4 text-sm font-medium text-slate-900">{formatRevenueDate(row.date)}</td>
                    <td className="px-5 py-4 text-sm text-slate-600">{row.orderCount}</td>
                    <td className="px-5 py-4 text-sm font-semibold text-slate-900">{formatCurrency(row.revenue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
